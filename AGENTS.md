...
> **Regla de cascada — las utilidades de marca van en `@layer components`.**
...
> que se pidió, pero si esas tarjetas se ven raras, el primer lugar donde mirar
> es el padding, no `.panel`.
>
> **Al agregar una utilidad de marca nueva, va dentro de `@layer components`.**
> Si queda suelta, el bug del botón reaparece.
...
app/
  ...
  icon.svg
  opengraph-image.tsx   # imagen de previsualización de links (1200x630)
...
lib/
  ...
  site.ts               # SITE: fuente única del dominio, nombre y textos públicos
...
- `app/sitemap.ts` y `app/robots.ts` los genera Next.js. El sitemap tiene **una
  sola URL** a propósito: las secciones son anclas y el protocolo de sitemaps no
...
- El JSON-LD (`components/site/structured-data.tsx`) declara solo
  ...
  agregues `address`, `telephone`, `sameAs` ni `offers` sin confirmación
  humana**: son datos que no están publicados, y `offers` volvería a duplicar
...
- ~~`package.json` todavía se llama `my-project`~~ — **resuelto**: ahora es `fuccina`.
- ~~Falta la imagen de Open Graph~~ — **resuelto**: `app/opengraph-image.tsx` la
  genera por código, 1200×630, con Geist desde `assets/`.
- **`images.unoptimized: true` sigue pendiente y vale la pena revisarlo:** los
  tres logos de integraciones pesan 67,6 kB en PNG (512×512, 320×320 y 640×480)
  y se muestran a **36×36 px**. Con `next/image` optimizado bajarían a unos pocos
  kB cada uno. Ojo: `sharp` no está instalado, así que hace falta en Vercel o
